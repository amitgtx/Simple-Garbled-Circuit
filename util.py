...
def specialEncryption(k, m):
    ...
    padded_m = b'\x00'*(KEYLENGTH//8) + m

    cip = b''.join( bytes([a^b]) for (a,b) in zip( padded_m, prf ) )
    # return pass
    c = r + cip
...
# def convert_byte_array_to_hex_string(a):
...
#         l = random.randint(16,48)
#         m = random_bytes(l)
#         c = specialEncryption(generate_key(), m)
#         m_dec = specialDecryption(k, c)

#         # assert m_dec == m

#         print (convert_byte_array_to_hex_string(m), "\t",
#          convert_byte_array_to_hex_string(c), "\t")
#         if (m_dec != None): print (convert_byte_array_to_hex_string(m_dec))
#         else: print ("NONE")
